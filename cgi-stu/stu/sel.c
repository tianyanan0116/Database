...
fclose(fd);
	...
	if (id[0] == '*')
	{
		sprintf(sql, "select id,sname,age,sex,sid from information where Sstatus=2");
	}
	else
	{
		sprintf(sql, "select id,sname,age,sex,sid from information where id = %d and Sstatus=2", atoi(id));
	}
